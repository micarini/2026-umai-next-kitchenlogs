...
const Card = ({ name, image, time = "35 MIN", level = "EASY" }) => {
	return (
		<article className="overflow-hidden rounded-2xl border-2 border-black/10 [background:var(--card-bg)] transition-transform duration-200 hover:-translate-y-1">
			<div className="aspect-4/3 w-full [background:var(--card-media-bg)]">
				{image ? (
					<img
						src={image}
						alt={name}
					...
					<div className="flex h-full w-full items-center justify-center text-sm font-semibold [color:var(--foreground)]/50">
						KitchenLogs
					</div>
				)}
			</div>
			...
			<div className="space-y-1 p-3">
				<div className="flex items-center gap-3 pb-1 pt-0.5">
					<span className="inline-flex items-center gap-[0.35em] [font-family:var(--font-dm-sans)] text-[0.78rem] font-bold uppercase">
						<svg viewBox="0 0 20 20" aria-hidden="true" className="h-[1.15em] w-[1.15em]">
					...
					<span className="inline-flex items-center gap-[0.35em] [font-family:var(--font-dm-sans)] text-[0.78rem] font-bold uppercase">
						<svg viewBox="0 0 50 50" aria-hidden="true" className="h-[1.15em] w-[1.15em]">
				...
				<h3 className="line-clamp-2 pb-1 [font-family:var(--font-roboto)] text-[1.45em] font-bold leading-[1.2] tracking-[0.01em] [color:var(--foreground)]">
					{name}
				</h3>
			</div>
		</article>
